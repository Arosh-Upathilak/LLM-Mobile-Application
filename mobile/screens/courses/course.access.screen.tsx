...
import { setAuthorizationHeader } from "@/hook/fetch/useUser";
...
export default function CourseAccessScreen() {
  ...
  const questionsFetchHandler = async () => {
    ...
    const response = await axios.get(
      `http://10.159.165.105:5000/api/course/get-questions/${courseContents[activeVideo].id}`,
    );
  ...
  const handleQandASubmit = async () => {
    await setAuthorizationHeader();
    setQuestionsLoader(true);

    /* await axios
      .post("http://10.159.165.105:5000/api/course/adding-qa", {
        question: question,
  ...
  const handleReviewSubmit = async () => {
    try {
      await setAuthorizationHeader();
      await axios.post("http://10.159.165.105:5000/api/course/add-review", {
        ratings: rating,
        review,
        courseId: params.id,
      });
